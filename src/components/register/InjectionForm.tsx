  ...
  return (
    <div className="space-y-4 animate-fade-in-up">
      <div>
        ...
        <input type="date" value={injDate} max={today} onChange={(e) => setInjDate(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-border bg-card text-sm outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" />
        {injDate > today && <p className="text-xs text-destructive mt-1">Você não pode registrar uma aplicação futura.</p>}
      </div>
      <div>
      ...
      <button onClick={handleSave} disabled={saving || !injDose || injDate > today}
