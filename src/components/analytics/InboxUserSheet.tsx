...
import {
  Syringe, Camera, Utensils, Dumbbell, ClipboardList, Mail, Copy, Check, Crown,
} from "lucide-react";
...
export const InboxUserSheet = ({ userId, onClose }: Props) => {
  ...
  return (
    <Sheet open={!!userId} onOpenChange={(o) => { if (!o) onClose(); }}>
      <SheetContent
        side="right"
        className="bg-slate-950 border-l border-slate-800 text-white w-full sm:max-w-md overflow-y-auto"
      >
        <SheetHeader className="text-left">
          <SheetTitle className="text-white text-base">
            {data?.profile?.name || "Sem nome"}
          </SheetTitle>
        </SheetHeader>
